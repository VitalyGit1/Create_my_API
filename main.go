...
type Response struct { /*
		нужна для того, чтобы описать, как будет выглядеть ответ нашего API. Сначала Go создаёт обычный объект (структуру), а уже потом превращает его в JSON.
		type - создаю новый тип данных.
		Response - имя нового типа.
		struct - Структура, которая объединяет несколько полей в один объект.
	*/
...
func handler(w http.ResponseWriter, r *http.Request) { /*
					Это параметры функции. Когда сервер вызывает handler, он автоматически передаёт ей два объекта:
					w http.ResponseWriter - объект, через который мы отправляем ответ клиенту. Всё, что записано в w, отправится пользователю. w можно представить
			как почтовый ящик для ответа. Всё, что ты запишешь в w, уйдёт пользователю. w (ResponseWriter) — записать и отправить ответ клиенту.
					r *http.Request - объект запроса. Внутри него хранится всё, что прислал клиент. r — это запрос, который прислал клиент. * - означает, что в функцию передаётся указатель на объект, а не его копия.
		r (Request) — прочитать, что попросил клиент.
	*/
...
func main() { //Это главная функция. Именно отсюда начинается выполнение программы
	...
	http.HandleFunc( //Регистрируем обработчик. То есть говорим серверу: Если придёт запрос на этот путь, вызывай функцию handler
		...
		handler,            //Это функция, которую нужно вызвать. Здесь ты говоришь серверу: Когда придёт запрос на /strings/length/, вызови функцию handler.
